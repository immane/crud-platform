...
#[AsCommand(name: 'app:trade:outbox:backfill-correlation', description: 'Backfill correlation IDs for unpublished Trade Outbox messages.')]
final class BackfillOutboxCorrelationCommand extends Command
{
    public function __construct(private readonly TradeOutboxMessageRepository $repository)
    {
        parent::__construct();
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$input->getOption('apply')) {
            $output->writeln(sprintf('Would backfill %d unpublished Trade Outbox message(s). Pass --apply to write changes.', count($ids)));
        ...
        $output->writeln(sprintf('Backfilled %d unpublished Trade Outbox message(s).', $updated));
